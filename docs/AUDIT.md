...
| MEDIUM `shim/guards.py` REGISTRY pinned to one literal | fixed | The entry now matches on the belief's own minting signature -- a `silent_ignore` anomaly on `update` IS "PATCH accepted a field it promised to reject" -- rather than on the field name, which is whatever key the caller happened to send. The fallback for a belief with no recorded signature reads `DOC_SPEC["promises"]["echo_fields"]` instead of restating it, since a second copy of the schema drifts silently. Negative controls: a belief carrying `nonexistent_field` compiles the guard; a `silent_coercion` belief on `update` whose evidence was `type_coercion` does not; the matching sentinel does not reach the exported manifest, which reports the field actually observed. Restoring the literal makes three of the five tests fail. |
| HIGH `bench/retire_demo.py::_check_page_size_cap` | fixed, and a larger fault found underneath | The seeding fix was correct but incomplete: the function also returned `still_capped` where `check_behaviour` is documented to return `world_matches_docs`, its negation. Verified live against an isolated lab with the rule on and then off -- it answered backwards in **both** directions, so the demo could never retire this belief. It now seeds past the cap, re-counts by paging under the cap (counting with a big page is circular when the cap is under test), returns `world_matches_docs`, and returns None when the population still cannot exercise the rule. `run_demo` treats a None round as inconclusive and moves no posterior. Negative controls: restoring the polarity fails two tests, removing the refusal fails a third, and a 12-row store gets no verdict. |
| LOW `shim/guards.py::IdempotentCreate.after` duplicate search | fixed | The discarded first `/v1/search` is gone; the guard now makes one. A test counts the client's POSTs and fails at two. |
| LOW `reflect/templates.py::ProbeTemplate.描述` | removed | Proven dead before removal: all seven constructors are keyword-only (so no positional shift), nothing reads the field, and no `probes/*.json` fixture or any other file in the repo mentions it. |
